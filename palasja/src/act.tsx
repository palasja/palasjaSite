...
import { fetchActInfo, fetchPersonalsByOrgId } from "./api";
...
const Act = ({orgId, month}:contractProps) => {
  ...
  return(
    ...
    <div>
      <p>Мы, нижеподписавшиеся: {`${LAST_NAME} ${ FIRST_NAME} ${MIDDLE_NAME}`}, с одной стороны и {`${sign?.lastName} ${sign?.firstName} ${sign?.middleName}`},  с другой стороны, составили настоящий акт в том, что первый выполнил  работы  в расчетно- справочном центре:</p>
    </div>
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {info?.services.map((s, i) => {
          ...
          <td>{s.count}</td>
          <td>{s.cost}</td>
          <td>{ ((s.count * s.cost)*100 + ((s.count * s.cost * (NDS/100))*100))/100}</td>
    ...
    <p>в полном объеме с {new Date(2025, month).toLocaleDateString("ru-RU")} по {new Date(2025, month+1, 0).toLocaleDateString("ru-RU")} согласно заключенного договора подряда.</p>
    <p>Акт составлен на предмет оплаты за выполненный объем работы, согласно заключенного договора подряда № {info?.contract.number} от {info?.contract.signDate.toString()}</p>
