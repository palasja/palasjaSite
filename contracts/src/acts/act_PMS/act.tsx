...
const currencyOption: ConvertOptions = {currency: {
...
let representor = info?.persons.find(p => new RegExp(REPRESENTOR_POSITION).test(p.positionName));
let economist = info?.persons.find(p => !new RegExp(REPRESENTOR_POSITION).test(p.positionName) && !p.isHead);
  return (
    <>
      {info?.contract === null ? (
        <>
          ...
          <div className={style.page}>
            ...
            <div className={style.main}>
              <p className={style.mainInfo}>
                Мы, стороны по договору от {new Date(info?.contract.signDate as unknown as string).toLocaleDateString('ru-RU')} года № {info?.contract.number}, Коммунальное мелиоративное унитарное предприятие «Наровлянское ПМС»,
                именуемое в дальнейшем «Заказчик», в лице  директора {head?.firstNameR} {head?.middleNameR} {head?.lastNameR}, действующего на основании Устава, 
                с одной стороны и гражданин Якубенко Иван Александрович, паспорт НВ 2955507, выданный 20.06.2016 г. Наровлянским РОВД, проживающий по адресу: 
                Гомельская область, г. Наровля, ул. Мелиоративная, 43/1, именуемый в дальнейшем «Исполнитель», составили настоящий акт о том, 
                что в соответствии с договором № {info?.contract.number} от {new Date(info?.contract.signDate as unknown as string).toLocaleDateString('ru-RU')} года Исполнителем выполнены следующие работы (оказаны услуги):
              </p>
              <p>- {info?.services.map(s => s.name).join(', ')}.</p>
              <p>Работы принял представитель Заказчика – {representor?.firstName} {representor?.middleName} {representor?.lastName}, {representor?.positionName}.</p>
              <p>Работы по договору выполнены на сумму {itog.toFixed(2)} ({convertNumberToWordsRu(itog, currencyOption)}). </p>
